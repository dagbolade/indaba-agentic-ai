...
scratch in Python, give it short-term memory and tools, and then add **persistent long-term memory
...
and the model runs privately on your machine. The only data that leaves is when you
deliberately point a tool at an external service (as in notebook `04`), and you choose which.
That is the point: agents you can host and control yourself are the foundation of
locally-owned, sovereign AI.
...
| [`04_agent_tools.ipynb`](notebooks/04_agent_tools.ipynb) | Give the agent a tool that calls a live weather API (no key) and let it use the real result | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/dagbolade/indaba-agentic-ai/blob/main/notebooks/04_agent_tools.ipynb) |
...
8. **Sovereign intelligence** (5 min): data, sovereignty, the path forward
9. **Q&A and resources** (5 min)
...
on open, self-hostable models: no metered API and no key, with the model running privately on
your machine. Anything the agent sends out goes only to the external tools you choose to give
it. That is the practical core of sovereign intelligence, and it is how locally-built,
